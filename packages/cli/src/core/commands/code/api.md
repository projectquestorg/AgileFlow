...
argument-hint: "[file|directory] [DEPTH=quick|deep|ultradeep] [FOCUS=conventions|errors|versioning|pagination|docs|all] [MODEL=haiku|sonnet|opus]"
compact_context:
  priority: high
  preserve_rules:
...
1. Show cost estimate: `node .agileflow/scripts/spawn-audit-sessions.js --audit=api --target=TARGET --focus=FOCUS --model=MODEL --dry-run`
2. Confirm with user before launching
3. Spawn sessions: `node .agileflow/scripts/spawn-audit-sessions.js --audit=api --target=TARGET --focus=FOCUS --model=MODEL`
