...
const { runEvent, applyOverride, resolveScriptPath, matcherMatches } =
  orchestratorModule;
...
describe("runEvent", () => {
  ...
  it("fails open (exitCode 0) when the chain has a runAfter cycle", async () => {
    // A malformed manifest (cycle) makes orderChain throw. The orchestrator
    // must fail open like the missing-manifest path — surfacing exit 1 would
    // spuriously error every hook invocation until the manifest is fixed.
    const agileflowDir = writeManifest(scratch, {
      version: 1,
      hooks: [
        { id: "a", event: "SessionStart", script: "a.js", runAfter: ["b"] },
        { id: "b", event: "SessionStart", script: "b.js", runAfter: ["a"] },
      ],
    });
    const result = await runEvent({ event: "SessionStart", agileflowDir });
    expect(result.exitCode).toBe(0);
      ...
      hooks: [
        {
          id: "welcome",
          event: "SessionStart",
          script: "plugins/core/hooks/welcome.js",
        },
      ],
    });
    const calls = [];
  ...
  describe("matcher filtering", () => {
    ...
    it("runs matcherless manifest hooks regardless of supplied matcher", async () => {
      const agileflowDir = writeManifest(scratch, {
        version: 1,
        hooks: [{ id: "all-tools", event: "PreToolUse", script: "a.js" }],
      });
      const calls = [];
      await runEvent({
        event: "PreToolUse",
  ...
  it("returns the concatenated stdout of successful hooks in chain order", async () => {
    const agileflowDir = writeManifest(scratch, {
      version: 1,
      hooks: [
        { id: "first", event: "SessionStart", script: "f.js" },
    ...
    const result = await runEvent({
      ...
      agileflowDir,
      runHook: makeRunHook({
        first: { exitCode: 0, stdout: "context from first\n" },
        second: { exitCode: 0, stdout: "context from second\n" },
      }),
    });
    expect(result.stdout).toBe("context from first\n\ncontext from second");
  });

  it("omits stdout of failed or empty hooks from the concatenated context", async () => {
    const agileflowDir = writeManifest(scratch, {
      version: 1,
      hooks: [
        { id: "ok-with-out", event: "SessionStart", script: "a.js" },
        {
          id: "ok-empty",
          event: "SessionStart",
          script: "b.js",
          runAfter: ["ok-with-out"],
        },
        {
          id: "failed",
          event: "SessionStart",
    ...
    const result = await runEvent({
      ...
      agileflowDir,
      runHook: makeRunHook({
        "ok-with-out": { exitCode: 0, stdout: "kept\n" },
        "ok-empty": { exitCode: 0, stdout: "   \n" },
        failed: { exitCode: 1, stdout: "dropped because failed\n" },
      }),
    });
    expect(result.stdout).toBe("kept");
  });

  it("returns an empty stdout string when there are no hooks", async () => {
    const agileflowDir = path.join(scratch, ".agileflow");
    const result = await runEvent({ event: "SessionStart", agileflowDir });
    expect(result.stdout).toBe("");
  });
  ...
  it("records timeout status when runHook reports timedOut: true", async () => {
    const agileflowDir = writeManifest(scratch, {
      version: 1,
      hooks: [
        { id: "slow", event: "SessionStart", script: "s.js", timeout: 100 },
      ],
    });
    const result = await runEvent({
      event: "SessionStart",
...
describe("resolveScriptPath", () => {
  it("joins relative scripts against the project root (parent of agileflowDir)", () => {
    ...
    expect(out.script).toBe(
      path.join(projectRoot, ".agileflow/plugins/core/hooks/welcome.js"),
    );
  });
