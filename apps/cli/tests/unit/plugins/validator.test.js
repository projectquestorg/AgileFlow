...
function valid(overrides = {}) {
  return {
    ...
    cannotDisable: true,
    depends: [],
    provides: { skills: [], agents: [], hooks: [], templates: [] },
    dir: "/x",
    ...overrides,
  };
}
...
describe("validatePlugin", () => {
  ...
  describe("booleans", () => {
    it("flags non-boolean enabledByDefault", () => {
      ...
      expect(
        issues.some((i) => /enabledByDefault.*boolean/.test(i.message)),
      ).toBe(true);
    });

    it("flags non-boolean cannotDisable", () => {
      const issues = validatePlugin(valid({ cannotDisable: 1 }));
      expect(issues.some((i) => /cannotDisable.*boolean/.test(i.message))).toBe(
        true,
      );
    });
    ...
    it("flags cannotDisable without enabledByDefault", () => {
      ...
      expect(
        issues.some((i) =>
          /cannotDisable: true.*enabledByDefault: true/.test(i.message),
        ),
      ).toBe(true);
    });
  });
  ...
  describe("depends", () => {
    ...
    it("flags self-dependency", () => {
      const issues = validatePlugin(valid({ depends: ["core"] }));
      expect(
        issues.some((i) => /cannot depend on itself/.test(i.message)),
      ).toBe(true);
    });
  ...
  describe("provides", () => {
    it("flags non-object provides", () => {
      const issues = validatePlugin(valid({ provides: ["skills"] }));
      expect(issues.some((i) => /must be an object/.test(i.message))).toBe(
        true,
      );
    });
    ...
    it("flags non-array sub-keys", () => {
      const issues = validatePlugin(
        valid({ provides: { skills: "skill.md" } }),
      );
      expect(issues.some((i) => /must be an array/.test(i.message))).toBe(true);
    });
    ...
    it("warns on unknown provides keys", () => {
      const issues = validatePlugin(
        valid({ provides: { skills: [], junk: [] } }),
      );
      expect(issues.some((i) => /not a recognized key/.test(i.message))).toBe(
        true,
      );
    });

    it("warns on legacy commands key", () => {
      const issues = validatePlugin(
        valid({ provides: { commands: [], skills: [] } }),
      );
      expect(issues.some((i) => /not a recognized key/.test(i.message))).toBe(
        true,
      );
...
describe("validatePluginSet", () => {
  ...
  it("flags duplicate plugin ids", () => {
    ...
    const issues = validatePluginSet(plugins);
    expect(issues.some((i) => /Duplicate plugin id/.test(i.message))).toBe(
      true,
    );
  });
  ...
  it("flags depends references that do not resolve", () => {
    ...
    const issues = validatePluginSet(plugins);
    expect(
      issues.some((i) => /unknown plugin "nonexistent"/.test(i.message)),
    ).toBe(true);
  });
});
...
describe("hasErrors", () => {
  it("returns true when any error-severity issue exists", () => {
    expect(
      hasErrors([{ severity: "error", pluginId: "a", message: "x" }]),
    ).toBe(true);
  });

  it("returns false for warnings-only", () => {
    expect(
      hasErrors([{ severity: "warning", pluginId: "a", message: "x" }]),
    ).toBe(false);
  });
