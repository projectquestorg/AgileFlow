...
describe("installPlugins integration", () => {
  ...
  it("installs core + selected opt-ins from bundled stubs in topological order", async () => {
    ...
    expect(fs.existsSync(path.join(agileflowDir, "plugins/council"))).toBe(
      false,
    );
  ...
  it("preserves unknown directories under plugins/ (does not blast user-placed content)", async () => {
    ...
    expect(fs.readFileSync(path.join(customDir, "note.txt"), "utf8")).toBe(
      "mine",
    );
  });
  ...
  it("aggregates plugin hooks into hook-manifest.yaml when ide=claude-code", async () => {
    ...
    expect(result.hookManifestPath).toBe(
    ...
    // With no `behaviors` filter passed, every behavior-gated hook the
    // core plugin declares lands in the manifest.
  ...
  it("filters hooks by the supplied behaviors map", async () => {
    ...
    expect(text).not.toMatch(/id: pre-compact-state/);
  });
  ...
  it("does NOT write hook-manifest.yaml when the IDE has hooks disabled", async () => {
    ...
    expect(fs.existsSync(path.join(agileflowDir, "hook-manifest.yaml"))).toBe(
      false,
    );
  });
  ...
  it("mirrors plugin skills to .claude/skills/<id>/ when ide=claude-code", async () => {
    ...
    const skillFile = path.join(
      scratch,
      ".claude/skills/agileflow-story-writer/SKILL.md",
    );
  ...
  it("rejects an invalid hook manifest before settings.json is touched", async () => {
    ...
    expect(fs.existsSync(path.join(scratch, ".claude", "settings.json"))).toBe(
      false,
    );
  });
