...
compact_context:
  ...
  preserve_rules:
    ...
    - "STEP 0: If args missing, use AskUserQuestion to ask TARGET, DEPTH, MODEL, FOCUS before starting"
...
**If DEPTH, MODEL, or FOCUS were NOT explicitly provided as arguments**, use `AskUserQuestion` before starting:

```xml
<invoke name="AskUserQuestion">
<parameter name="questions">[
  {
    "question": "Configure legal compliance audit — what should we scan?",
    "header": "Target",
    "type": "text",
    "key": "target",
  ...
  },
  {
    ...
    "multiSelect": false,
    "options": [
      {"label": "Quick — core analyzers, high-severity risks only (Recommended)", "value": "quick"},
      {"label": "Deep — all analyzers, includes medium-severity findings", "value": "deep"},
      {"label": "Ultradeep — each analyzer in its own tmux session (requires tmux)", "value": "ultradeep"}
    ]
  },
  {
    "question": "Which model should analyzers use?",
    "header": "Model",
    "multiSelect": false,
    "options": [
      {"label": "Haiku — fast and cheap (Recommended)", "value": "haiku"},
      {"label": "Sonnet — balanced quality and cost", "value": "sonnet"},
      {"label": "Opus — most thorough, highest cost", "value": "opus"}
    ]
  },
  {
    "question": "Focus on specific areas? (leave unselected for all)",
    "header": "Focus",
    "multiSelect": true,
    "optional": true,
    "options": [
      {"label": "privacy — GDPR, CCPA, cookie consent, data collection", "value": "privacy"},
      {"label": "terms — missing disclaimers, refund policies, contractual obligations", "value": "terms"},
      {"label": "a11y — ADA, Section 508, WCAG violations", "value": "a11y"},
      {"label": "licensing — copyleft violations, missing attribution, IP infringement", "value": "licensing"},
      {"label": "consumer — dark patterns, FTC violations, COPPA compliance", "value": "consumer"},
      {"label": "security — breach notification, PCI-DSS, encryption requirements", "value": "security"},
      {"label": "ai — EU AI Act, FTC AI disclosure, automated decision-making", "value": "ai"},
      {"label": "content — DMCA compliance, UGC platforms, Digital Services Act", "value": "content"},
      {"label": "international — LGPD, PIPL, data localization, cross-border transfers", "value": "international"}
    ]
  }
]</parameter>
</invoke>
...
**Skip this step if TARGET, DEPTH, MODEL, and FOCUS were all explicitly provided as arguments.**
