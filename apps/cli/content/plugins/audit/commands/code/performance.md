...
<parameter name="questions">[
  {
    "question": "Configure performance audit — what should we scan?",
    "header": "Target",
    "type": "text",
    "key": "target",
  ...
  },
  {
    ...
    "multiSelect": false,
    "options": [
      {"label": "Quick — core analyzers, high-impact issues only (Recommended)", "value": "quick"},
      {"label": "Deep — all analyzers, includes lower-severity findings", "value": "deep"},
  ...
  },
  {
    ...
    "optional": true,
    "options": [
      {"label": "queries — N+1, unindexed lookups, ORM anti-patterns", "value": "queries"},
      {"label": "rendering — unnecessary re-renders, expensive computations in render", "value": "rendering"},
      {"label": "memory — leaks, event listener cleanup, growing collections", "value": "memory"},
      {"label": "bundle — large imports, missing tree-shaking, duplicate deps", "value": "bundle"},
      {"label": "compute — blocking I/O, CPU-intensive loops, missing workers", "value": "compute"},
      {"label": "network — HTTP waterfall, missing batching, large payloads", "value": "network"},
      {"label": "caching — missing memoization, absent cache headers", "value": "caching"},
      {"label": "assets — unoptimized images, render-blocking resources", "value": "assets"}
    ]
  }
]</parameter>
</invoke>
