...
<parameter name="questions">[
  {
    "question": "Configure completeness audit — what should we scan?",
    "header": "Target",
    "type": "text",
    "key": "target",
  ...
  },
  {
    ...
    "multiSelect": false,
    "options": [
      {"label": "Quick — core analyzers, most impactful gaps only (Recommended)", "value": "quick"},
      {"label": "Deep — all analyzers, includes minor stubs and dead code", "value": "deep"},
  ...
  },
  {
    ...
    "optional": true,
    "options": [
      {"label": "handlers — empty onClick/onSubmit/onChange, noop callbacks", "value": "handlers"},
      {"label": "routes — placeholder hrefs, missing route targets, orphaned nav", "value": "routes"},
      {"label": "api — frontend-backend endpoint mismatches, missing handlers", "value": "api"},
      {"label": "stubs — TODO/FIXME, empty function bodies, hardcoded mock data", "value": "stubs"},
      {"label": "state — unused useState, orphaned context providers, dead store slices", "value": "state"},
      {"label": "imports — exported functions never imported, orphaned source files", "value": "imports"},
      {"label": "conditional — dead feature flags, hardcoded false conditions, unreachable code", "value": "conditional"}
    ]
  }
]</parameter>
</invoke>
