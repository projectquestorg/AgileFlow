...
<parameter name="questions">[
  {
    "question": "Configure security audit — what should we scan?",
    "header": "Target",
    "type": "text",
    "key": "target",
  ...
  },
  {
    ...
    "multiSelect": false,
    "options": [
      {"label": "Quick — core 5 analyzers, CRITICAL/HIGH only (Recommended)", "value": "quick"},
      {"label": "Deep — all 8 analyzers, includes MEDIUM/LOW findings", "value": "deep"},
  ...
  },
  {
    ...
    "optional": true,
    "options": [
      {"label": "injection — SQL, command, template injection", "value": "injection"},
      {"label": "auth — authentication flows and session handling", "value": "auth"},
      {"label": "authz — authorization, IDOR, privilege escalation", "value": "authz"},
      {"label": "secrets — hardcoded credentials, weak crypto", "value": "secrets"},
      {"label": "input — XSS, SSRF, file upload, prototype pollution", "value": "input"},
      {"label": "deps — known CVEs, typosquatting", "value": "deps"},
      {"label": "infra — Docker, headers, HTTPS, exposed endpoints", "value": "infra"},
      {"label": "api — mass assignment, rate limiting, GraphQL", "value": "api"}
    ]
  }
]</parameter>
</invoke>
