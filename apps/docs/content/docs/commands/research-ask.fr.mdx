...
Lorsque vous êtes bloqué sur un problème ou avez besoin de conseils d'experts, cette commande génère un **detailed 200+ line prompt** qui comprend :
...
Version du framework, dépendances et fichiers pertinents de votre base de code.

### 2. Mise en œuvre actuelle
**50+ lines of actual code** de votre projet, pas de pseudo-code ou de résumés.

### 3. Le problème
- Messages d'erreur exacts (textuellement)
- Trace de pile complète si disponible
- Comportement attendu et réel
- Étapes pour reproduire

### 4. Ce que nous avons déjà essayé
Au moins **2 approaches** avec des résultats détaillés et une analyse des raisons pour lesquelles ils n'ont pas fonctionné.

### 5. Questions spécifiques
**3+ targeted questions** responsables et spécifiques à votre situation.
...
## Intégration avec babysitting

Quand [`/agileflow:baby-sit`](/commands/babysit) détecte que vous êtes bloqué sur un problème, il suggère :
...
1. **Be specific with TOPIC** - Inclure les versions du framework et les types d'erreurs
2. **Include ERROR messages** - Les traces de pile complète aident énormément
3. **Let the command read your code** - Il extrait la mise en œuvre réelle
4. **Don't skip validation** - Les contrôles garantissent des réponses de qualité
