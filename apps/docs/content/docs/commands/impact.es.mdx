...
unAnaliza sólo los archivos enumerados (útil para el análisis manual).
...
### Omitir la ejecución de la prueba
...
|-----------|----------|---------|-------------|
| `FILES` | No | Detección automática | Rutas de archivos separadas por comas |
| `BASE` | No | principal/maestro | Rama base para comparar |
| `RUN_TESTS` | No | si | Ejecutar pruebas afectadas |

## Ejemplos

### Cambios típicos de funciones
...
- Analizar declaraciones de importación/requisito
- Construir dependencia graf
- Identificar dependencias circulares.
- Encuentra código exportado pero no utilizado
- Relaciones de archivos de mapas

**Beneficios**: Rápido, no requiere compilación/pruebas para ejecutarse, detecta cambios de importación
...
Coincidencia de patrones inteligente para pruebas relacionadas:

| Tipo de cambio | Patrón |
|-------------|---------|
| Ruta API cambiada | Busque `routes.test.ts`, `api/*.test.ts` |
| Componente cambiado | Busque `*.test.tsx`, `*.stories.tsx` |
| Servicio cambiado | Busque `service.test.ts`, pruebas de integración |
| Tipo cambiado | Busque `*.test.ts` que use el tipo |
...
El impacto sugiere qué pruebas to ejecutar:
...
### 2. Ejecutar pruebas (si RUN_TESTS=yes)
...
### serantes de fusionarse con principal
...
- [`/review`](/commands/review) - Revisión de calidad del código
- [`/verify`](/commands/verify) - Ejecutar pruebas
- [`/pr`](/commands/pr) - Generar relaciones públicas con notas de impacto
- [`/babysit`](/commands/babysit) - Guía de implementación (impacto de las ejecuciones)
