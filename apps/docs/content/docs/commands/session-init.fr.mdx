...
1. Crée le répertoire `.agileflow/sessions/` s'il est manquant
2. Initialise `registry.json` avec le schéma
...
- Ré-inscrivez-vous après des problèmes de système
...
- [`/session:new`](/commands/session-new) - Créer une session parallèle
