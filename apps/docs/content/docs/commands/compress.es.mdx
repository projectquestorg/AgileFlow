...
Reduzca el tamaño del archivo status.json eliminando los campos de historia detallados y al mismo tiempo conservando todos los metadatos de seguimiento críticos y manteniendo los detalles completos de la historia en archivos de rebajas.
...
Este comando resuelve un problema común: cuando `status.json` crece (especialmente en proyectos grandes con más de 100 historias), puede exceder los límites de tokens y ralentizar el procesamiento del agente. El comando de compresión:
- Elimina campos detallados (descripciones, aire acondicionado, contexto arquitectónico, etc.)
- Mantiene solo los metadatos de seguimiento esenciales (ID de la historia, estado, propietario, marcas de tiempo)
- Normalmente reduce el tamaño del archivo entre un 80 y un 90 %.
...
Este comando no tiene parámetros: opera directamente en `docs/09-agents/status.json`.
...
El comando:
1. Validar status.json existe y es JSON válido
2. Mostrar antes de las estadísticas (tamaño, líneas, recuento de historias)
3. Crear copia de seguridad automática
4. Eliminar campos detallados
5. Escribir versión comprimida
6. Mostrar después de estadísticas y ahorros
...
### Utilice compresión cuando:

1. **status.json exceeds 25,000 tokens**
   - Los agentes fallan con "el contenido del archivo excede los tokens máximos permitidos"
...
Para obtener resultados aún mejores, combine la compresión con el archivo:
...
Este proceso de dos pasos logra la máxima reducción manteniendo el historial.

## Seguridad y copias de seguridad

### Copia de seguridad automática

Antes de la compresión, el comando crea una copia de seguridad:
```text
...
- Sólo el índice status.json está comprimido
...
**status.json** = índice de seguimiento ligero
- "¿Qué historias existen?"
- "¿A quién pertenece cada historia?"
...
**docs/09-agents/archive/** = datos históricos
- Historias completadas anteriores al umbral
- organización mensual
- Capacidad de recuperación completa

### Beneficios

1. **Fast Indexing** - Los agentes leen status.json rápidamente
...
5. **Archivable** - Las historias antiguas no aumentan el índice de seguimiento.
...
### Con autobús de agente

La compresión status.json no afecta:
...
Se puede agregar compresión a la automatización de CI:
...
/agileflow:compress
```
**Result**: Se reduce a 30 KB, fácilmente por debajo de los límites de token
...
**Problem**: Quiere mantener status.json lean automáticamente
**Solution**: Agregar a la programación de CI (compresión mensual)
**Result**: Siempre bajo límites de token, procesamiento rápido del agente

## Solución de problemas

### "status.json sigue siendo demasiado grande después de la compresión"

Pruebe un archivo más agresivo:
```bash
...
### "Se perdió información crítica después de la compresión"
...
El contenido de la historia está en archivos de rebajas:
```bash
...
3. **Back Up Before** - La compresión crea una copia de seguridad automáticamente
4. **Verify After** - Verificar la estimación del token en la salida.
5. **Commit Changes** - Agregar estado comprimido a git
6. **Document Policy** - Registrar cuándo y por qué se ejecuta la compresión.
...
- Guión de archivo: `bash .agileflow/scripts/archive-completed-stories.sh` - Mover historias completadas para archivar
...
El script de compresión utiliza `jq` para procesamiento JSON:
...
Estimación aproximada utilizada en la salida:
- 1 token ≈ 4 caracteres/bytes (aproximado)
- Archivo de 100 KB ≈ 25 000 tokens
- Objetivo: \< 100 KB para mantenerse muy por debajo de los límites
...
<Accordion type="multiple">
  <AccordionItem value="delete">
    ...
    <AccordionContent>
      No. Sólo se modifica status.json. La rebaja de historias en docs/06-stories/ no se modifica.
    </AccordionContent>
  </AccordionItem>
  <AccordionItem value="undo">
    <AccordionTrigger>Can I undo compression?</AccordionTrigger>
    <AccordionContent>
      Sí. Se crea una copia de seguridad automáticamente. Simplemente restaure: `cp docs/09-agents/status.json.backup docs/09-agents/status.json`
    </AccordionContent>
  </AccordionItem>
  <AccordionItem value="when">
    <AccordionTrigger>When should I compress?</AccordionTrigger>
    <AccordionContent>
      Cuando status.json supera los 25 KB, o mensualmente como mantenimiento. Combínelo con archivo para obtener mejores resultados.
    </AccordionContent>
  </AccordionItem>
  <AccordionItem value="performance">
    <AccordionTrigger>Does this affect agent performance?</AccordionTrigger>
    <AccordionContent>
      Sí, lo mejora dramáticamente. Status.json más pequeño = lecturas más rápidas, más contexto disponible para los agentes.
    </AccordionContent>
  </AccordionItem>
  <AccordionItem value="git">
    <AccordionTrigger>What about git history?</AccordionTrigger>
    <AccordionContent>
      Todos los cambios están comprometidos. Las versiones anteriores están en el historial de git si es necesario.
    </AccordionContent>
  </AccordionItem>
  <AccordionItem value="customize">
    <AccordionTrigger>Can I customize which fields to keep?</AccordionTrigger>
    <AccordionContent>
      Sí, edite el script de compresión para conservar campos adicionales si es necesario para su flujo de trabajo.
    </AccordionContent>
  </AccordionItem>
</Accordion>
