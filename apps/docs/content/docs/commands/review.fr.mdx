---
title: "/revoir"
description: Révision de code basée sur l'IA avec suggestions de qualité
---

# /revoir

Effectuez une révision complète du code basée sur l’IA sur les modifications des demandes d’extraction. Review analyse le code pour en vérifier la qualité, la sécurité, les performances, les meilleures pratiques et fournit des commentaires exploitables avec des exemples de code spécifiques.

## Démarrage rapide

```bash
# Review current branch against main
/agileflow:review

# Review specific branch
/agileflow:review BRANCH=feature/auth

# Review with focus on security
/agileflow:review FOCUS=security
```

La commande va :
1. Obtenir les modifications de code (git diff)
2. Analyser à travers 6 catégories (Qualité, Sécurité, Performance, Meilleures pratiques, Tests, Documentation)
3. Générez un rapport détaillé avec les problèmes classés par gravité
4. Calculer le score de qualité du code
5. Suggérer des correctifs automatiques et des travaux de suivi

## Principales fonctionnalités

### Analyse complète

La révision examine les modifications de votre code dans 6 catégories importantes :

| Category | Checks |
|----------|--------|
| **Code Quality** | Complexity, duplication, naming, function/file length |
| **Security** | SQL injection, XSS, hardcoded secrets, unsafe deserialization |
| **Performance** | N+1 queries, inefficient algorithms, memory leaks |
| **Best Practices** | Error handling, logging, type safety, async/await |
| **Testing** | Missing tests, test quality, coverage gaps |
| **Documentation** | Missing JSDoc, outdated comments, README updates |

### Priorisation basée sur la gravité

Les problèmes sont classés par impact :

```text
🔴 CRITICAL - Must fix before merge
🟠 HIGH    - Fix before merge (blocks release)
🟡 MEDIUM  - Address soon (technical debt)
⚪ LOW     - Nice to have (style/consistency)
```

### Niveau de qualité du code

Votre PR obtient une note (0-100) avec répartition par catégorie :

```text
Code Quality Score: 72/100

Breakdown:
- Security: 40/100 (critical issues found)
- Performance: 75/100 (one N+1 query)
- Maintainability: 80/100 (some complexity)
- Testing: 65/100 (coverage gaps)
- Style: 90/100 (mostly consistent)
```

### Commentaires constructifs

Chaque numéro comprend :
- Où il se trouve (fichier, numéro de ligne)
- Pourquoi c’est important (risque/impact)
- Comment le réparer (exemples de code spécifiques)
- Exemple de ❌ MAUVAIS code
- Exemple de ✅ BON code

## Usage

### Basique – examiner la branche actuelle

```bash
/agileflow:review
```

Examine les modifications sur la branche actuelle par rapport à main/master.

### Examiner une branche spécifique

```bash
/agileflow:review BRANCH=feature/user-auth
```

Examine la branche nommée par rapport à main.

### Examen par rapport à une base différente

```bash
/agileflow:review BRANCH=feature/auth BASE=develop
```

Compare avec une branche de base différente.

### Révision avec zone de focus

```bash
# Security-only review
/agileflow:review FOCUS=security

# Performance-only review
/agileflow:review FOCUS=performance

# Testing coverage review
/agileflow:review FOCUS=tests
```

### Filtrer par gravité

```bash
# Show only critical/high issues
/agileflow:review SEVERITY=high

# Show all issues
/agileflow:review SEVERITY=all
```

## Paramètres

| Parameter | Required | Default | Options |
|-----------|----------|---------|---------|
| `BRANCH` | No | Current | Branch name to review |
| `BASE` | No | main/master | Base branch for comparison |
| `FOCUS` | No | all | all, security, performance, style, tests |
| `SEVERITY` | No | all | critical, high, medium, low, all |

## Exemples

### Revue de relations publiques typique

```bash
/agileflow:review

# Output:
# AI Code Review Report
# Branch: feature/user-auth
# Files Changed: 5
# Critical: 1 | High: 3 | Medium: 5 | Low: 2
# Code Quality Score: 72/100
#
# 🔴 CRITICAL ISSUES
# - Hardcoded API key in stripe.ts:15
#
# 🟠 HIGH ISSUES
# - N+1 query in posts API
# - Missing error handling in login
# ...
```

### Examen axé sur la sécurité

```bash
/agileflow:review FOCUS=security

# Analyzes only for:
# - Hardcoded secrets
# - SQL injection risks
# - XSS vulnerabilities
# - Authentication issues
# - CORS misconfiguration
```

### Évaluation des performances

```bash
/agileflow:review FOCUS=performance

# Checks for:
# - N+1 database queries
# - Inefficient algorithms
# - Missing indexes
# - Memory leaks
# - Bundle size issues
```

## Structure du rapport de révision

### En-tête

```text
# AI Code Review Report

**Branch**: feature/user-auth
**Base**: main
**Files Changed**: 8
**Lines Added**: 245
**Lines Removed**: 32
**Generated**: 2025-10-16T10:30:00Z
```

### Résumé

```text
## Summary

**Critical**: 2 | **High**: 5 | **Medium**: 12 | **Low**: 8
**Must Fix Before Merge**: 7 issues
```

### Problèmes par gravité

Pour chaque numéro :

```text
### 1. Hardcoded API Key (SECURITY)
**File**: src/api/payments/stripe.ts:15
**Severity**: CRITICAL

// ❌ BAD
const stripeKey = "sk_live_abc123...";

// ✅ GOOD
const stripeKey = process.env.STRIPE_SECRET_KEY;
if (!stripeKey) throw new Error("STRIPE_SECRET_KEY not set");

**Risk**: API key exposed in version control.
**Fix**: Move to environment variable. Rotate immediately.
**Priority**: Block merge
```

### Observations positives

```text
## Positive Observations ✅

- ✅ Good use of TypeScript strict mode
- ✅ Consistent error handling pattern
- ✅ Well-structured component separation
- ✅ Clear commit messages
```

### Répartition de la qualité du code

```text
## Code Quality Score: 72/100

**Breakdown**:
- Security: 40/100 (2 critical issues)
- Performance: 75/100 (1 N+1 query)
- Maintainability: 80/100 (some complexity)
- Testing: 65/100 (coverage gaps)
- Style: 90/100 (mostly consistent)
```

## Personnalisation

### Règles spécifiques au projet

Créez des règles de révision personnalisées dans :
- `.agileflow/review-rules.md`
- `docs/02-practices/code-standards.md`

Exemple:

```markdown
# Code Review Rules

## Critical
- No console.log in production code
- All API endpoints must have rate limiting
- All database queries must use ORM

## High
- Functions >30 lines should be refactored
- Test coverage must be >85%
```

## Actions après examen

### Problèmes de résolution automatique

```text
Review asks: "Fix auto-fixable issues? (YES/NO)"

If YES:
  - Runs npm run lint -- --fix
  - Fixes 8 style issues automatically
  - Shows which issues were fixed
```

### Créer des histoires de suivi

```text
Review asks: "Create stories for follow-up work?"

If YES:
  - Creates story for each medium/low issue
  - Adds to your backlog
  - Links to code review report
```

### Bloquer la fusion pour les problèmes critiques

```text
Review asks: "Block merge for critical/high issues?"

If YES:
  - Marks merge as blocked
  - Provides failure reason
  - Suggests resolution steps
```

### Enregistrer le rapport

Rapport automatiquement enregistré dans :
```text
docs/08-project/code-reviews/<YYYYMMDD>-<BRANCH>.md
```

Exemple:
```text
docs/08-project/code-reviews/20251016-feature-user-auth.md
```

## Contrôles de sécurité

La révision recherche automatiquement :

### Secrets et informations d'identification
- Clés API codées en dur
- Mots de passe de base de données
- Jetons OAuth
- Secrets JWT
- Informations d'identification AWS

### Vulnérabilités courantes
- Injection SQL (requêtes non paramétrées)
- Attaques XSS (sortie sans échappement)
- Désérialisation dangereuse
- Vulnérabilités CSRF
- Validation CORS manquante

### Modèles non sécurisés
- eval() ou Function()
- Concaténation de chaînes dangereuse
- Validation d'entrée manquante
- Faible cryptage
- Dépendances non sécurisées

## Vérifications des performances

L'examen recherche :

### Base de données
- Problèmes de requête N+1
- Index manquants
- Jointures inefficaces
- Requêtes inutiles

### Algorithmes
- O(n²) quand O(n) possible
- Boucles imbriquées qui pourraient être plates
- Calculs répétés
- Récursion non optimisée

### Mémoire
- Fuites de mémoire
- Objets volumineux non libérés
- Références circulaires
- Structures de données monopolisant la mémoire

## Couverture des tests

Vérifier les contrôles :

### Nouveau code
- Existe-t-il un test pour les nouvelles fonctionnalités ?
- Les cas extrêmes sont-ils couverts ?
- La qualité des tests est-elle bonne ?

### Lacunes de couverture
- Lignes ajoutées sans tests
- Fonctions sans couverture de test
- Chemins d'erreur non testés

### Qualité des tests
- Les affirmations ont-elles un sens ?
- Les tests vérifient-ils le comportement ?
- Les tests sont-ils maintenables ?

## Résultats et recommandations

### Actions immédiates (fusion de blocs)

```text
1. Remove hardcoded API key (critical)
2. Fix SQL injection (critical)
3. Add error handling to login (high)
```

### Avant la fusion

```text
4. Add tests for auth endpoints (high)
5. Fix N+1 query in posts API (high)
6. Reduce complexity in validator.ts (medium)
```

### Suivi (Créer des histoires)

```text
7. Standardize naming conventions (medium)
8. Add ESLint rule for secrets (medium)
9. Set up automated security scanning (low)
```

## Intégration avec CI

Suggérer d'ajouter à `.github/workflows/pr.yml`:

```yaml
- name: AI Code Review
  run: npx claude-code /agileflow:review BRANCH=${{ github.head_ref }}

- name: Check for critical issues
  run: |
    if grep -q "CRITICAL" code-review-report.md; then
      echo "::error::Critical issues found."
      exit 1
    fi
```

## Conseils

### Avant de soumettre le PR

```bash
/agileflow:review
# Fix any CRITICAL or HIGH issues
# Address MEDIUM issues if possible
# Then submit PR with confidence
```

### Amélioration itérative

```bash
# After feedback, run review again
/agileflow:review

# See if score improved
# Ensure critical issues fixed
# Check test coverage
```

### Approche axée sur la sécurité

```bash
# Always check security first
/agileflow:review FOCUS=security

# Fix any CRITICAL security issues
# Then review other categories
```

## Principes

- **Be constructive** - Fournissez des commentaires exploitables, pas seulement des critiques
- **Show examples** - Inclure ❌ MAUVAIS et ✅ BON code
- **Prioritize** - Les problèmes critiques d'abord, le style en dernier
- **Celebrate** - Inclure des observations positives
- **Suggest fixes** - Ne vous contentez pas d'identifier les problèmes
- **No force commits** - Toujours demander avant la réparation automatique

## Commandes associées

- [`/pr`](/commands/pr) - Générer une description du PR (examiner avant de soumettre)
- [`/impact`](/commands/impact) - Analyser l'impact du changement
- [`/garde d'enfants`](/commands/babysit) - Conseils de mise en œuvre (effectue un examen avant le PR)
- [`/vérifier`](/commands/verify) - Vérifier la réussite des tests
- [`/ligne de base`](/commands/baseline) - Créer un point de contrôle vérifié
