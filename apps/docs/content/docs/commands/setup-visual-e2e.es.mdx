...
**The problem**: Las pruebas pasan pero la interfaz de usuario no funciona.
...
1. **Check package.json** - Verificar que el proyecto tenga configuración npm
2. **Install Playwright** - `npm install --save-dev @playwright/test`
3. **Install browser** - `npx playwright install --with-deps chromium`
4. **Create config** - `playwright.config.ts` con servidor web
5. **Create test directory** - `tests/e2e/`
6. **Create screenshots directory** - `screenshots/`
7. **Create example test** - Prueba con captura de pantalla
8. **Add npm scripts** - `test:e2e` guiones
9. **Run verification** - Ejecutar prueba de ejemplo
...
Las pruebas generan capturas de pantalla en `screenshots/` directorio.
...
Las capturas de pantalla aprobadas cambian de nombre con `verified-` prefijo:

```bash
mv screenshots/homepage.png screenshots/verified-homepage.png
```

### 4. Ejecute el Verificador
...
Visual E2E se integra con [Modo de bucle](/features/loop-mode) para el desarrollo autónomo de UI:
...
1. Pasan las pruebas
2. Todas las capturas de pantalla tienen `verified-` prefijo
...
### 3. Nombra las capturas de pantalla de forma descriptiva
...
revisa tu `package.json` tiene el comando de desarrollo correcto:
...
### Tiempo de espera de las pruebas
...
- [`/agileflow:verificar`](/commands/verify) - Ejecutar pruebas y verificación.
- [`/agileflow:pruebas`](/commands/tests) - Pruebas de configuración de infraestructura.
