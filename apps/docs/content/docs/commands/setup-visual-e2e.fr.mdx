...
**Le problème** : les tests réussissent mais l'interface utilisateur est cassée.
...
1. **Vérifiez package.json** - Vérifiez que le projet a une configuration npm
2. **Installer Playwright** - `npm install --save-dev @playwright/test`
3. **Installer le navigateur** - `npx playwright install --with-deps chromium`
4. **Créer une configuration** - `playwright.config.ts` avec webServer
5. **Créer un répertoire de test** - `tests/e2e/`
6. **Créer un répertoire de captures d'écran** - `screenshots/`
7. **Créer un exemple de test** - Test avec capture d'écran
8. **Ajouter des scripts npm** - Scripts `test:e2e`
9. **Exécuter la vérification** - Exécuter un exemple de test
...
Les tests génèrent des captures d'écran dans le répertoire `screenshots/`.
...
### 3. Vérifier et renommer

Les captures d'écran approuvées sont renommées avec le préfixe `verified-` :
...
Visual E2E s'intègre au [Loop Mode](/features/loop-mode) pour le développement autonome de l'interface utilisateur :
...
2. Toutes les captures d'écran ont le préfixe `verified-`
...
|-----------|---------|-------------|
| DEV_CMD | `npm run dev` | Commande du serveur de développement |
| PORT | `3000` | Port du serveur de développement |
| NAVIGATEUR | `chromium` | Navigateur à installer (chrome est le plus petit, ~ 300 Mo) |
...
### 1. Capturer des pages complètes
...
Vérifiez que votre `package.json` a la bonne commande dev :
...
- [`/agileflow:verify`](/commands/verify) - Exécuter des tests et une vérification
