...
2. Mapper les dépendances (directes et indirectes)
3. Trouver des tests associés
4. Détecter les modifications avec rupture
...
### Basique : détection automatique des modifications
...
Analyse uniquement les fichiers répertoriés (utile pour l'analyse manuelle).
...
### Changement de middleware de sécurité
...
- Relations entre les fichiers de mappage

**Benefits**: Rapide, ne nécessite pas d'exécution de build/tests, détecte les modifications d'importation
...
### Sécurité de la refactorisation
...
- [`/révision`](/commands/review) - Revue de la qualité du code
- [`/vérifier`](/commands/verify) - Exécuter des tests
- [`/pr`](/commands/pr) - Générer des relations publiques avec des notes d'impact
- [`/garde d'enfants`](/commands/babysit) - Accompagnement de mise en œuvre (impact des runs)
