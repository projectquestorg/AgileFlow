...
### Problemas FALLIDOS
Muestra problemas críticos que deben solucionarse:
- Faltan secciones requeridas (AC, contexto arquitectónico)
...
- La sección existe con contenido real.
...
4. **Run again** - Volver a validar después de las correcciones
5. **Develop** - La historia estará lista cuando pasen todos los controles.
...
### Problema: "Los criterios de aceptación no están en formato Dado/Cuándo/Entonces"
**Fix**: Reescribe los criterios siguiendo el patrón:
```text
- Given [precondition]
  When [action]
...
**Fix**: Verifique que el archivo exista en `docs/04-architecture/` y utilice el anclaje de sección correcto.

### Problema: "La estimación del alcance parece alta"
**Fix**: Considere dividir la historia en partes más pequeñas o proporcionar una justificación en las Notas técnicas.
...
- **Before assignment**: Usar `/assign` sólo después de pasar la validación
- **Before sprint**: Usar `/sprint` con historias validadas
- **During development**: Informe de validación de referencia para el contexto.
...
- **Review failed checks carefully** - Cada uno tiene una sugerencia específica.
...
- **Test your stories** - La validación verifica la claridad, no la corrección.

## Comandos relacionados

- [`/historia`](/commands/story) - Crear nuevas historias
- [`/épico`](/commands/epic) - Crear épica para padres
