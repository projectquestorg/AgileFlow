...
```bash
/agileflow:discovery:new TOPIC="Dashboard" DEPTH=quick
/agileflow:discovery:new TOPIC="Analytics" DEPTH=deep
```
...
```bash
/agileflow:discovery:new TOPIC="Dark mode toggle" DEPTH=quick
```
...
```bash
/agileflow:discovery:new TOPIC="OAuth integration" DEPTH=guided
```
...
```bash
/agileflow:discovery:new TOPIC="Microservices migration" DEPTH=deep
```
...
Delegates to `/discovery:brief` to:
...
```bash
# Run quick discovery
/agileflow:discovery:new TOPIC="Dark mode support" DEPTH=quick
...
/agileflow:discovery:new TOPIC="Analytics Dashboard" DEPTH=guided
...
/agileflow:discovery:new TOPIC="Microservices Replatforming" DEPTH=deep
...
```bash
# Initial discovery
/discovery:new TOPIC="Mobile app" DEPTH=quick

# Gather feedback
# Review brief and identify gaps

# Refine and regenerate
/discovery:brief TOPIC="Mobile app" RESEARCH=docs/10-research/user-feedback.md

# Or start fresh with more research
/discovery:new TOPIC="Mobile app" DEPTH=deep
```
...
/agileflow:discovery:new TOPIC="Add export to CSV" DEPTH=quick
```
...
/agileflow:discovery:new TOPIC="..." DEPTH=guided  # (default)
```
...
/agileflow:discovery:new TOPIC="Rewrite auth system" DEPTH=deep
```

## Related Commands

- [`/discovery:brief`](/commands/discovery-brief) - Synthesize findings into brief (standalone)
