...
AgileFlow has over 90 commands and 47 agents. Most users only know about a handful. Smart Detection solves this discovery problem by:
...
4. **Recommending only what matters** - Surfacing 2-5 contextual suggestions, not all 90 commands
...
```
obtain-context.js (gather project data)
        |
        v
smart-detect.js (orchestrator)
    |           |
    v           v
lifecycle-     signal-
detector.js    detectors.js
    |           |
    v           v
Detect phase   Run 42 detectors
(5 phases)     against signals
        |
        v
smart-detect.json (recommendations)
        |
        v
babysit reads and acts on recommendations
```
