...
- `high` (12 agents): Preserved in most compactions
  - CI, DevOps, Testing, Security, Documentation, Performance, Codebase-Query, Orchestrator, Team-Coordinator, Error-Analyzer, Code-Reviewer, Multi-Expert
  - Run long-duration work with sustained focus
- `medium` (30 agents): Preserved when possible
  - Specialized agents (analytics, integrations, mobile, etc.) plus logic/legal/security/perf/test analyzers
  - Handle domain-specific tasks
- `low` (10 agents): First to be removed
  - Validators, configuration agents, epic-planner
