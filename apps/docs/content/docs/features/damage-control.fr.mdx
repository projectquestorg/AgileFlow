...
Damage Control est un système de sécurité qui empêche les agents IA d'exécuter des commandes destructrices ou dangereuses. Il utilise celui de Claude Code **PreToolUse hooks** pour valider les commandes avant exécution.
...
- Destruction de base de données : `DROP TABLE`, `TRUNCATE TABLE`, `DELETE FROM ... ;`
...
- La base de données est supprimée avec WHERE : `DELETE FROM table WHERE ...`
- Édition: `npm publish`
...
- AgileFlow v2.78.0+
- Claude Code (dernier)
- `.agileflow` répertoire installé

### Étape 1 : Choisissez le niveau de protection

**Standard (Recommended)**
- Correspondance de modèle rapide uniquement
- Latence de 0 ms ajoutée
- Plus de 40 schémas dangereux bloqués
- Parfait pour la plupart des projets

**Enhanced**
- Évaluation Standard + IA pour les menaces inconnues
...
### Modifier un exemple de crochet
...
### Philosophie d'ouverture en cas de panne

Si le fichier de modèles est corrompu ou manquant, les commandes sont **allowed** (non bloqué). Cela garantit qu'un système de sécurité défectueux ne bloque pas toutes les opérations.
...
R : Modifier `.agileflow/config/damage-control-patterns.yaml`:
...
R : Pas facilement. Les modèles correspondent aux chaînes de commande réelles. Des solutions de contournement simples comme `r m -rf` créer des erreurs de syntaxe.
...
1. Fermer complètement (Cmd+Q / Ctrl+Q)
...
- Utiliser la protection standard (la protection améliorée est expérimentale)
