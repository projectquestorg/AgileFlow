...
**Priority Levels**:
- `critical` (4 agents) : Toujours conservé même en compactage agressif
  - Mentor, API, UI, Database
  - Gérer l'orchestration et la mise en œuvre du noyau
- `high` (14 agents) : Préservé dans la plupart des compactages
...
- `medium` (27 agents) : Préservé lorsque cela est possible
  - Spécialistes autonomes, agents du council et coordinateurs de consensus
  - Gérer les tâches spécifiques au domaine
- `low` (66 agents) : Premier à être supprimé
  - Sous-analyseurs d'audit (logic-*, legal-*, security-*, perf-*, test-*, completeness-*, seo-*, ads-*, brainstorm-*), validateurs
  - Agents éphémères du pipeline d'audit, inclus uniquement si l'espace le permet
