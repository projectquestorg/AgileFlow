...
description: Install and configure AgileFlow for Claude Code, Cursor, Windsurf, Codex CLI, or Antigravity.
---
...
AgileFlow currently supports five IDEs:
...
| **OpenAI Codex CLI** | `.codex/` | CLI tool | Supported | - |
| **Antigravity** | `.antigravity/` | Agentic dev platform | Supported | - |
...
Damage control is enabled during interactive setup when you choose the hook options.
...
4. **Hooks** (project-level): Codex lifecycle hooks in `.codex/config.toml`
   - Enabled with `codex_hooks = true`
   - Uses `hooks.json` or inline `[hooks]` tables
   - Supports the same AgileFlow session, tool, and stop dispatchers Codex exposes
...
Configure Codex for full access:
...
**Problem**: Dangerous commands aren't being blocked.
  ...
  **Solution**:
  ...
  2. Verify `.claude/settings.json` has PreToolUse hooks
