...
Le mode Boucle permet un traitement autonome des histoires via le système **Ralph Loop** d'AgileFlow. Nommé d'après le modèle « Ralph Wiggum » issu des recherches d'Anthropic sur les flux de travail agents, il traite automatiquement les histoires dans une épopée sans intervention manuelle.

Le mode boucle est activé via la commande [`/agileflow:babysit`](/commands/babysit) avec des paramètres spéciaux. Babysit orchestre l'ensemble du processus tandis que Ralph Loop gère l'automatisation.
...
3. Lorsque Claude s'arrête, exécute automatiquement les tests
4. Si les tests réussissent → marque l'histoire terminée, charge l'histoire suivante
5. Continue jusqu'à ce que l'épique soit terminée ou que les itérations maximales soient atteintes

Cela permet le **traitement par lots de nuit** d'epics bien définis avec des critères d'acceptation clairs.
...
Le mode Boucle utilise le système de crochet d'arrêt de Claude Code. Quand Claude arrête de répondreng :

1. Le script `ralph-loop.js` s'exécute automatiquement
...
| `MAX` | Non | 20 | Itérations maximales avant l'arrêt |
...
- Exécute `npm test` après chaque arrêt de Claude
...
- Vérifie le répertoire `screenshots/` pour les images
- Toutes les captures d'écran doivent avoir le préfixe `verified-`
...
- Échec si la couverture descend en dessous du seuil

## Sélection d'histoires

Le mode Boucle sélectionne les histoires dans cet ordre de priorité :

1. **Histoire actuelle** - Si une est déjà en cours
2. **Histoires prêtes** - Histoires avec le statut « prêt » (toutes les dépendances sont remplies)
3. **Histoires bloquées** - Ignorées jusqu'à ce qu'elles soient débloquées
...
### Configuration du crochet d'arrêt

Le mode boucle nécessite le crochet d'arrêt dans `.claude/settings.json` :
...
- ✅ Tâches répétitives d'implémentation (points de terminaison CRUD, composants)
...
Si testEn cas d'échec, le mode boucle affiche les échecs et continue de fonctionner :
...
3. Assurez-vous que des tests existent pour l'histoire actuelle
...
### 1. Commencez par un petit EPics
...
### 5. Utiliser le mode visuel pour l'interface utilisateur
...
- [`/agileflow:babysit`](/commands/babysit) - La commande mentor qui orchestre le mode boucle
- [`/agileflow:verify`](/commands/verify) - Vérification manuelle des tests
...
| Composant | Objectif |
|-----------|---------|
| `ralph-loop.js` | Script d'orchestration de la boucle principale |
| `screenshot-verifier.js` | Vérification de la capture d'écran du mode visuelroi |
| `session-state.json` | Configuration et état de la boucle |
| Crochet d'arrêt | Point d'intégration Claude Code |
